...
type logStreamHub struct {
	mu          sync.RWMutex
	nextID      uint64
	subscribers map[int64]map[uint64]chan *StepEvent
}

func newLogStreamHub() *logStreamHub {
	return &logStreamHub{subscribers: make(map[int64]map[uint64]chan *StepEvent)}
}
...
func (h *logStreamHub) Subscribe(jobLogID int64) (<-chan *StepEvent, func()) {
	if h == nil {
		return nil, func() {}
	}
	...
	if _, ok := h.subscribers[jobLogID]; !ok {
		h.subscribers[jobLogID] = make(map[uint64]chan *StepEvent)
	}
...
func (h *logStreamHub) Publish(jobLogID int64, event *StepEvent) {
	if h == nil || event == nil {
		return
	}
