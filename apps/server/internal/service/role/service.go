...
func New(repo *rolerepo.Repository, menuRepo *menurepo.Repository) *Service {
	if repo == nil {
		return nil
	}
...
func (s *Service) ListRoles(ctx context.Context, opts ListOptions) (*ListResult, error) {
	if s == nil || s.repo == nil {
	...
	records, total, err := s.repo.ListRoles(ctx, rolerepo.ListOptions{
		PageNum:  pageNum,
		PageSize: pageSize,
