...
const useResponsiveContext = () => {
  ...
  if (!context) {
    throw new Error('Responsive dialog components must be used within <ResponsiveDialogRoot>');
  }
  return context;
};
...
function ResponsiveDialogRoot({
  ...
  const value = React.useMemo<ResponsiveDialogContextValue>(() => ({ isMobile }), [isMobile]);
