...
interface NoticeAppliedFiltersProps {
  items: FilterChip[];
  onRemove: (key: string) => void;
  onClear: () => void;
}

export function NoticeAppliedFilters({
  items,
  onRemove,
  onClear,
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-lg bg-muted/40 px-4 py-3">
      ...
      <Button
        type="button"
        variant="link"
        className="inline-flex items-center gap-1 px-0 text-sm text-destructive hover:text-destructive"
        onClick={onClear}
      >
