...
export function MenuPermissionTree({ nodes, value, onChange, disabled }: MenuPermissionTreeProps) {
  const flatNodes = useMemo(() => {
    const entries: Array<{ node: MenuTreeNode; parentId: number }> = [];
  ...
  const allIds = useMemo(() => flatNodes.map((entry) => entry.node.menuId), [flatNodes]);

  const parentMap = useMemo(() => {
    const map = new Map<number, number>();
    flatNodes.forEach(({ node, parentId }) => {
  ...
  const expandedAll = useMemo(() => parentIds.every((id) => expanded.has(id)), [expanded, parentIds]);
